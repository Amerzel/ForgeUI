...
describe('Accordion', () => {
  it('renders all trigger labels', () => {
    render(<Themed><Accordion items={ACCORDION_ITEMS} /></Themed>)
  ...
  it('collapses open item on second click (collapsible=true)', async () => {
    const user = userEvent.setup()
    render(<Themed><Accordion items={ACCORDION_ITEMS} collapsible /></Themed>)
    await user.click(screen.getByText('Section A'))
    await user.click(screen.getByText('Section A'))
    expect(screen.queryByText('Content A')).not.toBeInTheDocument()
  })
  ...
  it('controlled: shows value item open', () => {
    render(<Themed><Accordion items={ACCORDION_ITEMS} value="b" onValueChange={vi.fn()} /></Themed>)
    expect(screen.getByText('Content B')).toBeVisible()
  })

  it('multiple type: can open multiple items', async () => {
    const user = userEvent.setup()
    render(<Themed><Accordion type="multiple" items={ACCORDION_ITEMS} /></Themed>)
    await user.click(screen.getByText('Section A'))
  ...
  it('has no a11y violations (closed)', async () => {
    const { container } = render(<Themed><Accordion items={ACCORDION_ITEMS} /></Themed>)
    expect(await axe(container)).toHaveNoViolations()
  })

  it('has no a11y violations (open)', async () => {
    const { container } = render(
      <Themed><Accordion items={ACCORDION_ITEMS} value="a" onValueChange={vi.fn()} /></Themed>
    )
    expect(await axe(container)).toHaveNoViolations()
  })
})
...
describe('Tabs', () => {
  it('renders all tab labels', () => {
    render(<Themed><Tabs items={TAB_ITEMS} /></Themed>)
  ...
  it('calls onValueChange when tab changes', async () => {
    ...
    render(<Themed><Tabs items={TAB_ITEMS} onValueChange={onValueChange} /></Themed>)
    await user.click(screen.getByRole('tab', { name: 'Tab 2' }))
    expect(onValueChange).toHaveBeenCalledWith('two')
  })

  it('controlled: shows specified value tab', () => {
    render(<Themed><Tabs items={TAB_ITEMS} value="two" onValueChange={vi.fn()} /></Themed>)
    expect(screen.getByRole('tab', { name: 'Tab 2' })).toHaveAttribute('data-state', 'active')
  })

  it('renders solid variant without error', () => {
    render(<Themed><Tabs items={TAB_ITEMS} variant="solid" /></Themed>)
    expect(screen.getByRole('tablist')).toBeInTheDocument()
  })

  it('renders vertical orientation', () => {
    render(<Themed><Tabs items={TAB_ITEMS} orientation="vertical" /></Themed>)
    expect(screen.getByRole('tablist')).toHaveAttribute('aria-orientation', 'vertical')
  })

  it('has no a11y violations', async () => {
    const { container } = render(<Themed><Tabs items={TAB_ITEMS} /></Themed>)
    expect(await axe(container)).toHaveNoViolations()
  })
})

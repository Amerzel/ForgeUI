...
const meta: Meta = {
  ...
  tags: ['autodocs'],
  parameters: {
    docs: { description: { component: 'Zoom/pan image viewer with pixel-perfect rendering, checkerboard transparency, and keyboard shortcuts.' } },
    layout: 'fullscreen',
  },
}
export default meta
  ...
  render: () => (
    <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        <Text size="sm" style={{ fontWeight: 500 }}>Texture Preview</Text>
        <Badge>256 × 256</Badge>
      </div>
      <ImageViewer
        src={DEMO_IMAGE}
        alt="Demo texture"
        checkerboard
        style={{ height: '400px', border: '1px solid var(--forge-border)', borderRadius: 'var(--forge-radius-md)' }}
      />
...
export const PixelArt: Story = {
  ...
  render: () => {
    ...
    const pixelArt = 'data:image/svg+xml,' + encodeURIComponent(
    ...
    return (
      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Text size="sm" style={{ fontWeight: 500 }}>Pixel Art Sprite</Text>
          <Badge>8 × 8</Badge>
          <Text size="xs" style={{ color: 'var(--forge-text-muted)', marginLeft: 'auto' }}>Pixelated rendering preserves crisp edges</Text>
        </div>
        <ImageViewer
          src={pixelArt}

...
const meta: Meta = {
  ...
  tags: ['autodocs'],
  parameters: {
    docs: { description: { component: 'Node-based graph editor built on ReactFlow. For shader graphs, behaviour trees, quest flows.' } },
    layout: 'fullscreen',
  },
}
export default meta
...
const INITIAL_NODES: FlowNode[] = [
  { id: 'tex-albedo',    type: 'default', position: { x: 60, y: 80 },   data: { label: '🖼 Albedo Texture' } },
  { id: 'tex-normal',    type: 'default', position: { x: 60, y: 200 },  data: { label: '🗺 Normal Map' } },
  { id: 'tex-roughness', type: 'default', position: { x: 60, y: 320 },  data: { label: '◻ Roughness Map' } },
    ...
    return (
      <div style={{ height: '520px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        <NodeEditor
          ...
          aria-label="Shader graph editor" style={{ flex: 1 }}
        />
      </div>
    )
  },
    ...
    return (
      <div style={{ height: '380px', padding: '16px' }}>
        <NodeEditor
          nodes={nodes} edges={edges}
          onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect}
          minimap={false} aria-label="Behaviour tree editor"
        />
      </div>
    )
  },

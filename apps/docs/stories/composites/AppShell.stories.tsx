...
export const Default: Story = {
  ...
  parameters: {
    layout: 'fullscreen',
    docs: {
      description: { story: 'Full-viewport CSS grid layout. Scroll down to see the panel layout.' },
    },
  },
  render: () => (
    <div style={{ height: '600px', minWidth: '1280px', overflow: 'hidden' }}>
      <AppShell
        nav={
          ...
          >
            <span style={{ color: 'var(--forge-accent)', fontWeight: 700, fontSize: '14px' }}>
              ⚒ ForgeUI
            </span>
        ...
        sidebar={
          <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            {['Root', '  Camera', '  Player', '    Mesh', '    Collider', '  Lights'].map(
              (item) => (
                <div
                  key={item}
                  style={{
                    ...
                    color: item === '  Player' ? 'var(--forge-accent)' : 'var(--forge-text-muted)',
                    backgroundColor:
                      item === '  Player' ? 'var(--forge-surface-hover)' : 'transparent',
                    cursor: 'default',
                  }}
                >
                  {item}
